...
def get_period_mask(df: pd.DataFrame, date_col: str, year: int) -> pd.Series:
    ...
    year=2026: 2026-01-01 ~ max (即 >= 2026-01-01)
    """
...
def calculate_store_tenure_metrics(df):
    ...
    Returns bin statistics for 2025 and 2026.
    """
    results = {}
...
def calculate_metrics(df: pd.DataFrame) -> dict:
    ...
    for metric_name in metric_names:
        ...
        # 2026 数据
        mask_2026 = get_metric_mask(metric_name, 2026)
        count_2026 = df[mask_2026]['order_number'].nunique()
        
        # 同比 (YoY) - 注意：2026可能不完整，这里仅计算简单增长率供参考，或者留空
        yoy = ((count_2026 - count_2025) / count_2025) if count_2025 > 0 else 0.0
    ...
    for metric_name in metric_names:
        ...
        for s in target_series:
            ...
            # 2026
            mask_2026 = series_mask & get_metric_mask(metric_name, 2026)
            val_2026 = df[mask_2026]['order_number'].nunique()
            
            # Diff & Ratio
            diff = val_2026 - val_2025
            ratio = (diff / val_2025) if val_2025 > 0 else 0.0
        ...
        total_2025 = sum(row["2025 全年"] for row in rows)
        total_2026 = sum(row["2026 (至今)"] for row in rows)
        total_diff = total_2026 - total_2025
        total_ratio = (total_diff / total_2025) if total_2025 > 0 else 0.0
    ...
    for s in target_series:
        ...
        for metric_name in target_energy_metrics:
            ...
            for energy_type, condition_str in product_type_logic.items():
                ...
                # 2026
                mask_2026 = combined_mask & get_metric_mask(metric_name, 2026)
                val_2026 = df[mask_2026]['order_number'].nunique()
    ...
    # 6. 锁单未交付未退订分布 (Pending Delivery Distribution - 2026)
    # 过滤: lock_time >= 2026-01-01 (using 2026 logic), approve_refund_time is null, invoice_upload_time is null
    mask_2026_lock = get_period_mask(df, "lock_time", 2026)
    ...
    if has_refund_col and has_invoice_col:
        ...
        pending_mask = mask_2026_lock & mask_no_refund & mask_no_invoice
    ...
    # 合并 2025 和 2026 数据，仅生成一个总体分布
    
    mask_period_2025 = get_period_mask(df, "invoice_upload_time", 2025)
    mask_period_2026 = get_period_mask(df, "invoice_upload_time", 2026)
    
    # Combined mask
    mask_period = mask_period_2025 | mask_period_2026
    ...
    try:
        ...
        if 'owner_age' in df.columns:
            ...
            for s in target_series:
                ...
                # 2026
                mask_2026 = series_mask & get_metric_mask("留存锁单数", 2026)
                mean_2026 = calc_mean_age(mask_2026)
                
                # Diff
                diff = mean_2026 - mean_2025
                diff_str = f"{diff:+.1f}"
                if diff < 0:
            ...
            mask_total_2026 = get_metric_mask("锁单数", 2026)
            mean_total_2026 = calc_mean_age(mask_total_2026)
            
            diff_total = mean_total_2026 - mean_total_2025
            ...
            if 'parent_region_name' in df.columns:
                ...
                for region in regions:
                    ...
                    # 2026
                    mask_2026 = region_mask & get_metric_mask("锁单数", 2026)
                    mean_2026 = calc_mean_age(mask_2026)
            ...
            # Use 'gender' field.
            # Calculate counts for 2025 vs 2026.
            # Output:
        ...
        if gender_col is not None:
            ...
            for s in target_series:
                ...
                # Pre-calculate masks for 2025 and 2026
                mask_2025 = series_mask & get_metric_mask("留存锁单数", 2025)
                mask_2026 = series_mask & get_metric_mask("留存锁单数", 2026)
                ...
                data_2026 = data_2026.dropna(subset=['std_gender'])
                
                counts_2025 = data_2025['std_gender'].value_counts()
                counts_2026 = data_2026['std_gender'].value_counts()
                
                total_2025 = counts_2025.sum()
                total_2026 = counts_2026.sum()
                
                for g_label in ['男', '女']:
                    c_2025 = counts_2025.get(g_label, 0)
                    c_2026 = counts_2026.get(g_label, 0)
                    
                    share_2025 = (c_2025 / total_2025) if total_2025 > 0 else 0.0
                    share_2026 = (c_2026 / total_2026) if total_2026 > 0 else 0.0
                    
                    diff_share = share_2026 - share_2025
            ...
            if 'parent_region_name' in df.columns:
                ...
                for region in regions:
                    ...
                    mask_2026 = region_mask & get_metric_mask("留存锁单数", 2026)
...
def calculate_conversion_probability(metrics):
    """
    根据历史(2025+2026)的退订和交付分布，计算各时长区间的'生存转化率'。
    ...
    # 1. Aggregate Historical Data (2025 + 2026)
    # Note: Module 2.1 data structure is a list of dicts [{'year': 2025, 'counts': ...}, ...]
...
def generate_html(metrics: dict, output_file: Path):
    ...
    html_content = [
        ...
        "<head>",
        "<meta charset='utf-8'>",
        "<title>2025 vs 2026 业务指标对比分析</title>",
        css,
        "</head>",
        "<body>",
        "<h1>2025 vs 2026 业务指标对比分析</h1>",
    ...
    html_content.append("<li>2025: 2025-01-01 ~ 2025-12-31</li>")
    html_content.append("<li>2026: 2026-01-01 ~ 至今 (Max Date)</li>")
    html_content.append("</ul>")
    ...
    if 'pending_delivery_dist' in metrics:
        ...
        layout = get_common_layout(
            title=f"2.2 锁单未交付未退订分布 (2026) - Total: {total_count:,}",
        ...
        html_content.append(f"<h3>2.2 锁单未交付未退订分布 (2026)</h3>")
        html_content.append(f"<p>统计范围: 2026年锁单，截至当前 ({pending_data['data_timestamp'].strftime('%Y-%m-%d')}) 既未退订也未交付的订单。</p>")
    ...
    if 'delivery_cycle_dist' in metrics:
        ...
        if total > 0:
            ...
            layout = get_common_layout(
                title=f"锁单交付周期分布 (2025 & 2026) - Total: {total:,}",
                xaxis_title="Duration (Days)",
                yaxis_title="Percentage"
            )
    ...
    if 'pending_delivery_dist' in metrics:
        ...
        if conv_prob is not None:
            ...
            layout = get_common_layout(
                title=f"2.4 锁单未交付预估 (2026) - Total Est Conversion: {int(total_est_deliveries):,}",
                xaxis_title="Duration (Days since Lock)",
            ...
            html_content.append("<h3>2.4 锁单未交付预估 (2026)</h3>")
    ...
    if 'delivery_overview' in metrics:
        ...
        for label, key in metrics_to_show:
            ...
            val_2026 = data_2026.get(key, 0.0)
    ...
    if 'active_store_series' in metrics:
        ...
        def get_diff_ratio(metric_key):
            v24 = raw_stats[2025][metric_key]
            v25 = raw_stats[2026][metric_key]
        ...
        s_2026 = s_active[s_active.index.year == 2026]

        # 4.2 Total Operating Days Analysis (Moved from 4.3)
        html_content.append("<h3>4.2 营业总时长分析 (Total Operating Days Analysis)</h3>")
        html_content.append("<p>统计2025年和2026年所有在营门店的营业天数总和 (Sum of operating days for all active stores).</p>")
        
        # Calculate Total Operating Days
        total_days_2025 = int(s_2025.sum()) if not s_2025.empty else 0
        total_days_2026 = int(s_2026.sum()) if not s_2026.empty else 0
        
        # Calculate Diff and Ratio
        diff_days = total_days_2026 - total_days_2025
        
        if total_days_2025 != 0:
            ratio_days = diff_days / total_days_2025
        else:
            ratio_days = np.nan
        ...
        # 2026 Trace
        if not s_2026.empty:
            x_2026 = s_2026.index.dayofyear
            dates_2026 = s_2026.index.strftime('%Y-%m-%d')
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate="Day %{x} (%{customdata})<br>Active Stores: %{y}<extra>2026</extra>",
                customdata=dates_2026
            ))
            
        layout = get_common_layout(
            title="4.3.1 在营门店数趋势对比 (2025 vs 2026)",
            xaxis_title="年份天数 (Day of Year)",
            yaxis_title="在营门店数"
        )
        layout['xaxis']['range'] = [1, 366] # Explicitly set range to match 2025
        ...
        if 'store_tenure_analysis' in metrics:
            ...
            colors = {2025: '#3498DB', 2026: '#E67E22'}
    ...
    html_content.append("<p>统计2025年和2026年每日的“当日锁单数”除以“当日在营门店数”。</p>")
    ...
    if 'module5_stats' in metrics:
        ...
        for label, key, fmt in row_configs:
            ...
            stats_2026 = m5_stats.get(2026)
            
            v2025 = stats_2025.get(key, 0) if stats_2025 else 0
            v2026 = stats_2026.get(key, 0) if stats_2026 else 0
            
            # Calculate Diff and Ratio (YoY)
            diff = v2026 - v2025
            
            if v2025 != 0:
                ratio = (v2026 - v2025) / v2025
            else:
                ratio = np.nan
