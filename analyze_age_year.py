...
OUTPUT_HTML = SCRIPT_DIR / "reports/age_timing_heatmap.html"
...
def main():
    ...
    mask_2025 = (df['lock_time'] >= '2025-01-01') & (df['lock_time'] <= '2025-12-31')

    # 2026: >= 2026-01-01
    mask_2026 = (df['lock_time'] >= '2026-01-01')
    ...
    print(f"2026 Total Records: {len(df_2026)}")
    ...
    df_2026['age_group'] = df_2026['age'].apply(lambda x: map_age_group(x, 2026))
    ...
    for year, sub_df in [(2024, df_2024), (2025, df_2025), (2026, df_2026)]:
        # Ensure numeric
    ...
    titles = ["2024 销量占比", "2025 销量占比", "2026 销量占比"]
    for s in series_list:
        titles.extend([f"2024 {s} 均价", f"2025 {s} 均价", f"2026 {s} 均价"])
    ...
    for i, (year, sub_df) in enumerate([(2024, df_2024), (2025, df_2025), (2026, df_2026)]):
    ...
    layout_update = {
        "title_text": "2024-2026 年龄段购车洞察：时间偏好(Row 1) 与 各车系开票均价(Rows 2-4)",
        "height": 1800,
        "width": 1800, 
