下面是当前仓库里所有 `analyze_*.py` 脚本的用途说明，按数据源分类，方便查询。

## 一、基于 `intention_order_analysis.parquet` 的分析脚本

此组脚本主要使用 `intention_order_analysis.parquet` 作为核心数据源（部分脚本可能辅以其他数据），侧重于用户画像、渠道归因、配置偏好及因果推断等深度分析。

**1. scripts/analyze_plot_dash_align.py**  
...
**2. scripts/analyze_plot_owner_age_price.py**  
...
**3. scripts/analyze_causal_forest_battery_effect.py**  
...
**4. scripts/analyze_user_to_config_rules.py**  
...
**5. scripts/analyze_product_types_preference_by_city.py**  
...
**6. scripts/analyze_plot_channel_share_comparison.py**  
...
此组脚本主要使用 `order_full_data.parquet`（全量订单/开票数据）、`leads_daily_*.csv`（线索数据）或特定业务记录表，侧重于宏观销量、价格趋势、竞品 PK 及 OCR 监控等分析。
...
**7. scripts/analyze_2026.py**  
面向年度复盘的“2026 经营 Review”总控脚本，基于 `analyze_2025.py` 迭代而来。主要用于 2025 与 2026 年度的对标分析，统计逻辑与 2025 版保持一致，但将对比周期调整为 2025 全年 vs 2026 至今。生成 Plotly HTML 报告（`review_2026.html`），用于跟踪 2026 年的最新经营表现及同比变化。

**8. scripts/analyze_age_year.py**  
基于 `order_full_data.parquet` 的锁单明细，先过滤“留存锁单”（退订为空），再用业务定义中的 `series_group_logic` 和 `time_periods` 划分子车型与时间窗，并映射到主系列（LS6、L6、LS9 等）。脚本按年份（2024、2025、2026）和系列，将车主年龄分箱为“00后、95后、90后...”等年龄段，再结合锁单月份或时间段构建二维分布（热力图），输出 `age_timing_heatmap.html`，用于观察不同年龄代际的购车高峰时间与车型偏好差异。

**9. scripts/analyze_plot_leads_normality.py**  
...
**10. scripts/analyze_plot_leads_aggregated_comparison.py**  
...
**11. scripts/analyze_multi_period_kpi.py**  
多指标双时间窗对比分析脚本，结合 `assign_data.csv`（线索下发数据）和 `order_full_data.parquet`（订单数据）。支持通过命令行参数指定两个时间窗口（Start1-End1 vs Start2-End2），对比分析线索下发量、门店转化效率（Conversion Efficiency）、线索分布比例（Distribution Ratio）等 KPI 指标的变化。输出 HTML 报告，用于复盘不同营销活动或时间段内的线索与转化效能差异。
